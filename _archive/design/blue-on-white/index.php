...
Some options: */
$autostart = true; // true/false -- should it start playing automatically? Regardless, it’ll never autoplay when accessing a permalinked image
$autodelay = 100; // int -- when auto-advancing, how long should we view each image (ms)
...
<head>
	...
	<title>White Whale Web Services</title>
	<style type="text/css">
		/* Tweak these styles */
		body { background-image:none; background-color:#fff; } /* Any body styles needed for background images, etc */
	...
	if(images.length>1) { // if there's more than one image
		...
		play.click(function() { // Auto advance
			...
			if($(this).is('.playing')) { // if we're now paused
				...
				},settings.delay+10); // at specified interval + 600 (for the fade)
			} else {
